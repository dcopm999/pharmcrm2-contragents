...
def coverage(c):
    ...
    """
    c.run("coverage run --source pharmcrm2-contragents runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/pharmcrm2-contragents.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ contragents")
...
def lint(c):
    ...
    """
    c.run("flake8 pharmcrm2-contragents tests")
...
def release(c, bumpsize=""):
    ...
    c.run(
        "git tag -a {version} -m 'New version: {version}'".format(
            version=pharmcrm2_contragents.__version__
        )
    )
    c.run("git push --tags")
